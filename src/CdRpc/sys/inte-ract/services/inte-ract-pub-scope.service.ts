...
export class InteRactPubService extends GenericService<InteRactPubModel> {
    ...
    async create(req: Request, res: Response) {
        ...
        if (await this.validateCreate(req, res)) {
            ...
            console.log('InteRactPubService::create()/account:', account)
            const serviceInput = {
            ...
            console.log('InteRactPubService::create()/afterResult:', result)
    ...
    async createI(req, res, serviceInputExt: IExtServiceInput<any>): Promise<InteRactPubModel | boolean> {
        console.log('InteRactPubService::createI()/createI()/01')
        if (await this.validatecreateI(req, res, serviceInputExt)) {
            console.log('InteRactPubService::createI()/02')
            const account = await this.beforeCreateI(req, res, serviceInputExt);
            console.log('InteRactPubService::createI()/account:', account)
            const result = this.b.createI(req, res, serviceInputExt)
            const afterResult = await this.afterCreate(req, res, result)
            console.log('InteRactPubService::createI()/afterResult:', afterResult)
            return afterResult;
        } else {
            const r = await this.b.respond(req, res);
        }
     ...
     * @returns 
     */
    async afterCreate(req, res, newBill: InteRactPubModel): Promise<any> {
        
    }

    async read(req: Request, res: Response, serviceInput: IServiceInput<any>): Promise<any> {
        const q = this.b.getQuery(req);
        console.log('InteRactPubService::getBill/q:', q);
        try {
    ...
    async getInteRactPub(req: Request, res: Response) {
        ...
        const serviceInput = {
            serviceModel: InteRactPubModel,
            docName: 'InteRactPubService::getBill',
            cmd: {
                action: 'find',
                query: q
            },
    ...
    getPaged(req: Request, res: Response) {
        const q = this.b.getQuery(req);
        console.log('InteRactPubService::getBillCount()/q:', q);
        const serviceInput = {
            serviceModel: InteRactPubModel,
            docName: 'InteRactPubService::getBillCount',
            cmd: {
                action: 'find',
                query: q
            },
