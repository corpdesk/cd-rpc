...
import { InteRactLinkModel } from '../models/inte-ract-link.model';
import { from, Observable } from 'rxjs';
import { getConnection } from 'typeorm';
import { GenericController } from "../../base/generic-controller";
import { GenericService } from "../../base/generic-service";

// export class InteRactLinkService extends CdService {
export class InteRactLinkService extends GenericService<InteRactLinkModel> {
    err: string[] = []; // error messages
    b: any; // instance of InteRactLinkService
    cdToken: string;
    serviceModel = InteRactLinkModel;
    docName = 'InteRactLinkService';
    sessModel;
    ...
    async create(req: Request, res: Response) {
        console.log('InteRactLinkService::create()/01')
        const svSess = new SessionService();
        if (await this.validateCreate(req, res)) {
            const account = await this.beforeCreate(req, res);
            console.log('InteRactLinkService::create()/account:', account)
            const serviceInput = {
                serviceInstance: this,
                serviceModel: InteRactLinkModel,
                serviceModelInstance: this.serviceModel,
                docName: 'Create InteRactLink',
                dSource: 1,
            }
            const result = await this.b.create(req, res, serviceInput)
            console.log('InteRactLinkService::create()/afterResult:', result)
            const afterResult = await this.afterCreate(req, res, result)
            console.log('InteRactLinkService::create()/afterResult:', afterResult)
            this.b.i.app_msg = '';
    ...
    async createI(req, res, serviceInputExt: IExtServiceInput<any>): Promise<InteRactLinkModel | boolean> {
        console.log('InteRactLinkService::createI()/createI()/01')
        if (await this.validatecreateI(req, res, serviceInputExt)) {
            console.log('InteRactLinkService::createI()/02')
            const account = await this.beforeCreateI(req, res, serviceInputExt);
            console.log('InteRactLinkService::createI()/account:', account)
            const result = this.b.createI(req, res, serviceInputExt)
            const afterResult = await this.afterCreate(req, res, result)
            console.log('InteRactLinkService::createI()/afterResult:', afterResult)
            return afterResult;
        } else {
            const r = await this.b.respond(req, res);
        }
    ...
    async validatecreateI(req, res, serviceInputExt) {
        console.log('InteRactLinkService::validateCreateI()/01')
        let countInvalid = 0;
        // const validRequired = await this.b.validateRequired(req, res, this.cRules)
        // console.log('InteRactLinkService::validateCreateI()/validRequired:', validRequired)
     ...
     * @returns 
     */
    async afterCreate(req, res, newBill: InteRactLinkModel): Promise<any> {
        
    }

    async read(req: Request, res: Response, serviceInput: IServiceInput<any>): Promise<any> {
        const q = this.b.getQuery(req);
        console.log('InteRactLinkService::getBill/q:', q);
        try {
            this.b.read$(req, res, serviceInput)
                .subscribe((r: any) => {
                    // console.log('InteRactLinkService::read$()/r:', r)
                    this.b.i.code = 'InteRactLinkService::Get';
                    const svSess = new SessionService();
            ...
            console.log('InteRactLinkService::read$()/e:', e)
            this.b.err.push(e.toString());
            const i = {
                messages: this.b.err,
                code: 'InteRactLinkService:update',
                app_msg: ''
            };
    ...
    async update(req: Request, res: Response) {
        console.log('InteRactLinkService::update()/01');
        let q = this.b.getQuery(req);
        q = this.beforeUpdate(q);
        const serviceInput = {
            serviceModel: InteRactLinkModel,
            docName: 'InteRactLinkService::update',
            cmd: {
                action: 'update',
                query: q
            },
        ...
        console.log('InteRactLinkService::update()/02')
    ...
    async updateI(req, res, q): Promise<any> {
        console.log('InteRactLinkService::updateI()/01');
        // let q = this.b.getQuery(req);
        q = this.beforeUpdate(q);
        const serviceInput = {
            serviceModel: InteRactLinkModel,
            docName: 'InteRactLinkService::updateI',
            cmd: {
                action: 'update',
                query: q
            },
    ...
    async validateCreate(req: Request, res: Response) {
        console.log('InteRactLinkService::validateCreate()/01')
        const validRequired = await this.b.validateRequired(req, res, this.cRules)
        console.log('InteRactLinkService::validateCreate()/validRequired:', validRequired)
    ...
    //  * @param res
    //  */
    async getInteRactLink(req: Request, res: Response) {
        const q = this.b.getQuery(req);
        console.log('InteRactLinkService::getBill/q:', q);
        const serviceInput = {
            serviceModel: InteRactLinkModel,
            docName: 'InteRactLinkService::getBill',
            cmd: {
                action: 'find',
                query: q
            },
    ...
    getPaged(req: Request, res: Response) {
        const q = this.b.getQuery(req);
        console.log('InteRactLinkService::getBillCount()/q:', q);
        const serviceInput = {
            serviceModel: InteRactLinkModel,
            docName: 'InteRactLinkService::getBillCount',
            cmd: {
                action: 'find',
                query: q
            },
    ...
    async delete(req: Request, res: Response) {
        const q = this.b.getQuery(req);
        console.log('InteRactLinkService::delete()/q:', q)
        const serviceInput = {
            serviceModel: InteRactLinkModel,
            docName: 'InteRactLinkService::delete',
            cmd: {
                action: 'delete',
                query: q
            },
