...
// import { CD_FX_FAIL, type ICdResponse, type ISessResp } from '../../base/i-base';
...
//    * Updates the session data in the `cd-cli.profileson` file under the appropriate profile.
//    */
  ...
  async saveSession(session: ISessResp, profileName: string): Promise<boolean> {
    try {
      ...
      const jwtToken =
        session.jwt && typeof session.jwt === 'object'
          ? session.jwt.jwtToken
          : (session.jwt as unknown as string);
    ...
    try {
      ...
      if (
        ...
        (session as any).jwtObj = session.jwt
          ...
          : null;
        session.jwt = (session as any).jwtObj;
      }
