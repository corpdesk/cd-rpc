...
export class CdPdf {
    ...
    async fromUrl(req: Request, res: Response) {
     ...
     * @param res 
     */
    async fromHtml(req: Request, res: Response) {
        const browser = await puppeteer.launch()
        const page = await browser.newPage()
        ...
        // console.log('CdPdf::fromHtml()/(req as any).post.dat.f_vals[0]:', (req as any).post.dat.f_vals[0].pdfHtml)
        let htmlContent = (req as any).post.dat.f_vals[0].data.pdfHtml;
        // console.log('CdPdf::fromHtml()/htmlContent:', htmlContent)
        let csss:string[] = (req as any).post.dat.f_vals[0].data.pdfCss;
    ...
    async withCss(req: Request, res: Response) {
        const browser = await puppeteer.launch();
        const page = await browser.newPage();
        // add css
        const contents = `
