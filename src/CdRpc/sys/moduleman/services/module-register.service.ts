...
import { CdCtx, CdModuleDescriptor } from '../../dev-descriptor/index';
...
// import { EnvCreate, EnvPurge } from '../models/module.model';
...
export class ModuleRegisterService {
  ...
  async registerModuleInCdInstance(moduleData: CdModuleDescriptor): Promise<CdFxReturn<null>> {
    try {
      ...
      const response = await this.http.proc(EnvCreate, "cd-api-local");
  ...
  async deRegisterModuleFromCdInstance(moduleData: CdModuleDescriptor): Promise<CdFxReturn<null>> {
    try {
      ...
      const response = await this.http.proc(EnvPurge, "cd-api-local");
