...
export class CrudTestService {
  ...
  //       ? record.result.state
  //       : record.result.state === CdFxStateLevel.Success;
  //   const log: ITestLog = {
  //     timestamp: new Date().toISOString(),
  //     category: success ? 'response' : 'error',
  ...
  getRequest(c: CdControllerDescriptor): ICdRequest {
    ...
    const model = this.module.models?.[c.name as any];
    const pk = model?.primaryKey ?? `${toCamelCase(c.name)}Id`;
    const query: IQuery = { where: { [pk as any]: 1 } };
